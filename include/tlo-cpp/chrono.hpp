...
// tmToTimestamp(). Returns the timestamp. Thread-safe when called in multiple
// threads concurrently. Thread-safe when called concurrently with
// timeToUtcTimestamp(). Not thread-safe when called concurrently with other
...
// tmToTimestamp(). Returns the timestamp. Thread-safe when called in multiple
// threads concurrently. Thread-safe when called concurrently with
// timeToLocalTimestamp(). Not thread-safe when called concurrently with other
// functions that also call std::localTime(), std::gmtime(), or std::ctime().
